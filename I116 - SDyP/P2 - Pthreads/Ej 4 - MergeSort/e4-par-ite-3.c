...
void* ordenarIterativo(void* arg){
    ...
    const int inicio = id * lenHilo;    // 0

    // Etapa 1: ordenación de la porción asignada
    int lenTrabajo, L, M ,R;
    int fin = (id+1) * lenHilo - 1;     // 29

    // 2, 4, 8, 16
    for (lenTrabajo=2; lenTrabajo <= lenHilo; lenTrabajo *= 2){
        // En ultimo len: L = 0, 16 // M = 7, 23 // R = 15, 29
        for (L=inicio; L < fin; L += lenTrabajo){
            M = L + lenTrabajo/2 - 1;
            if (M >= fin) break;    // ya está ordenado

            R = min(L + lenTrabajo - 1, fin);
            combinar(L, M, R);
        }
    }
