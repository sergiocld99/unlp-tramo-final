...
void p0(int cp, int N){
    ...
    double* CL = (double*) malloc(filas*N*sizeof(double));
    ...
    MPI_Gather(CL, filas*N, MPI_DOUBLE, C, filas*N, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    //for (i=1; i<cp; i++){
        //elementoInicio = i * elementosProceso;
...
void p1(int cp, int N){
    ...
    MPI_Scatter(vacio, 0, MPI_DOUBLE, A, filas*N, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Bcast(B, N*N, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    ...
    // enviar resultados
    MPI_Gather(C, filas*N, MPI_DOUBLE, vacio, 0, MPI_DOUBLE, 0, MPI_COMM_WORLD);
