...
void matmul(double* a, double* b, double* c, int stripSize, int n, int bs, int debug){
  int i,j,k;

  // paralelizamos con 8 hilos
  #pragma omp for schedule(static) nowait
	for (i=0; i<stripSize; i+=bs){
    // if (debug) printf("hilo %d, i = %d en %lf \n", omp_get_thread_num(), i, MPI_Wtime());
	  for (j=0; j<n; j+=bs){
		  for (k=0; k<n; k+=bs){
...
int main(int argc, char* argv[]){
  ...
  #pragma omp parallel
  {
    matmul(a, b, r1, stripSize, n, bs, rank == 0);
    matmul(c, d, r2, stripSize, n, bs, rank == 0);
    matmul(e, f, r3, stripSize, n, bs, rank == 0);
  }
