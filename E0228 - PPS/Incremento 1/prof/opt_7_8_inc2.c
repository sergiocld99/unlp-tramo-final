...
void floydWarshall(TYPE* D, int* P, int n, int t){
	...
	for(k=0; k<r; k++) {
		...
		for (i=0; i<r; i++)
			semaforos[k][i] = (sem_t*) malloc(r * sizeof(sem_t));
	}
	...
	#pragma omp parallel default(none) shared(semaforos) firstprivate(r,row_of_blocks_disp,num_of_bock_elems,D,P) num_threads(t)
	{
		...
		for(k=0; k<r; k++){
			...
			if (k!=0) // Esperar que se compute el bloque (k,k) de la fase anterior
				sem_wait(&semaforos[k][k][k]);
			
			FW_BLOCK_PARALLEL(D, kk, kk, kk, P, b, tmp1, tmp2);
			
			// avisar a (k,k) de la siguiente fase
			sem_post(&semaforos[k+1][k][k]);
			...
			for(w=0; w<r*2; w++){
				if(w<r){ //Phase 2
					...
					if (k!=0) // Esperar que se compute el bloque (k,j) de la fase anterior
						sem_wait(&semaforos[k][k][j]);
										
					FW_BLOCK(D, kj, kk, kj, P, b, tmp1, tmp2);
					
								// avisar a (k,j) de la siguiente fase
					sem_post(&semaforos[k+1][k][j]);
				...
				/*	for (aux=0; aux< r; aux++)
					...
					if (k!=0) // Esperar que se compute el bloque (i,k) de la fase anterior
						sem_wait(&semaforos[k][i][k]);
					...
					for (aux=0; aux< r; aux++)
						if(aux != k)
							sem_post(&semaforos[k][i][aux]);

					// -------------- FIN BLOQUE AGREGADO -----------------									

					
								// avisar a (k,k) de la siguiente fase
					sem_post(&semaforos[k+1][i][k]);
			...
			for(i=0; i<r; i++){
				for(j=0; j<r; j++){
					...
					if (k!=0) // Esperar que se compute el bloque (i,j) de la fase anterior
						sem_wait(&semaforos[k][i][j]);
					...
					// avisar a (i,j) de la siguiente fase
					sem_post(&semaforos[k+1][i][j]);
