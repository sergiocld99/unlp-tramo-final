;; variables de las turtles

;; trenes: estado 0 (yendo), 1 (en anden), 2 (lejano)
trenes-own [ estado espera t_anden ]

;; pasajeros: estado 0 (vagando), 1 (cruzando puente), 2 (yendo a tren)
...
to crear_estacion
  ;; crear via
  if pxcor = -2 or pxcor = -4 or pxcor = 2 or pxcor = 4
  [set pcolor gray]

  ;; durmientes
  if abs(pxcor) = 3 or abs(pxcor) = 1 or abs(pxcor) = 5 and (pycor mod 2 = 0)
  [set pcolor brown]

  ;; linea del anden
  if pxcor = 6 or pxcor = -6
  [set pcolor yellow]
  ...
  ;; puente
  if pycor > 9 and pycor < 12 and abs(pxcor) < 8
  [set pcolor red]
end
...
to crear_tren
  ...
  ifelse who = 0
  [
    ;; tren que va hacia el norte (plaza)
    set espera frecuencia_tren / 2
    set heading 0
    set xcor -3 ]
  [
    ;; tren que va hacia el sur
    set espera frecuencia_tren
    set heading 180
    set xcor 3 ]
end
...
to mover_pasajero
  ...
  ifelse abs(xcor) < 7 or abs(xcor) > max-pxcor - 1
